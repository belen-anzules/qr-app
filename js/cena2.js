...
const ingredients = {
    pollo: { name: "Pechuga Grill", price: 1.5, calories: 165, protein: 31, fat: 3.6, carbs: 0, qty: 1, included: true },
    lechuga: { name: "Mix Lechugas", price: 0.5, calories: 15, protein: 1.2, fat: 0.2, carbs: 2.8, qty: 1, included: true },
    queso: { name: "Parmesano", price: 0.8, calories: 110, protein: 10, fat: 7, carbs: 1, qty: 1, included: true }
};
...
function cargarConfiguracion() {
    ...
    if (params.has('editIndex')) {
        ...
        for (let k in ingredients) { if (params.has(k)) ingredients[k].qty = parseInt(params.get(k)); }
    }
    updateUI();
}
function updateUI() {
    ...
    for (let key in ingredients) {
        ...
        if(document.getElementById(`${key}-qty`)) {
            ...
            totalCal += ing.calories * ing.qty; totalPro += ing.protein * ing.qty;
            totalFat += ing.fat * ing.qty; totalCarb += ing.carbs * ing.qty;
...
function confirmarPedido() {
    ...
    let extrasArray = []; let configActual = {};
    ...
    if (editIndex !== null) cart[editIndex] = item; else cart.push(item);
